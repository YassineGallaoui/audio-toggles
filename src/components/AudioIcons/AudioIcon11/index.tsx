...
const AudioIcon11: React.FC<AudioIconProps> = React.memo(({ audioOn }) => {
    ...
    const generateSineWavePath = useCallback((t: number, amp: number) => {
        ...
        for (let x = -10; x <= 10; x += step) {
            const y = amp * Math.sin(x * 0.5 + t * 5);
    ...
    const [newPathData1, newPathData2] = useMemo(() => {
        return [
            ...
            generateSineWavePath(t, amplitude * 0.8)
        ];
...
AudioIcon11.displayName = "AudioIcon11";

export default AudioIcon11;